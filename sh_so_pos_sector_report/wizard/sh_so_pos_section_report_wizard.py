...
class SectionReportWizard(models.TransientModel):
    ...
    total_amount = fields.Selection(
        [("untax_amount", "Untax Amount"), ("total_amount", "Total Amount")],
        string="Total Amount",
    ...
    def print_section_report(self):
        ...
        if not self.included_sale_order and not self.included_pos_orders:
            raise UserError(
                "Please Tick included sale or pos order to print the report"
            )
        ...
        bold_center = xlwt.easyxf(
            ...
            "borders: top thin,bottom thin,right thin,left thin"
        )
        ...
        left_bold = xlwt.easyxf(
            "align: horiz left;font:bold True;"
            "borders: top thin,bottom thin,right thin,left thin"
        )
        right_bold = xlwt.easyxf(
            "align: horiz right;font:bold True;"
            "borders: top thin,bottom thin,right thin,left thin"
        )
        ...
        currency_style.num_format_str = (
            str(self.company_id.currency_id.symbol) + "#,##0.00;"
        )
        ...
        currency_style_sided.num_format_str = (
            str(self.company_id.currency_id.symbol) + "#,##0.00;"
        )
        ...
        first_day = (today - timedelta(days=today.weekday())) - timedelta(
            days=7 * (self.total_weeks - 1)
        )

        self.env.cr.execute(
            """select name,from_time,to_time from sh_sale_pos_sector order by sequence;"""
        )
        ...
        for week_count in range(int(range_value)):
            ...
            if res:
                for sector in res:
                    ...
                    for i in range(display_week_no):
                        ...
                        date_start = day_start_day.astimezone(
                            pytz.timezone("UTC"))
                        ...
                        if self.included_pos_orders:
                            ...
                            self.env.cr.execute(
                                """ select ROUND(sum(o.amount_total),2) from pos_order as o 
                        ...
                        if self.included_sale_order:
                            ...
                            if self.total_amount == "total_amount":
                                ...
                                self.env.cr.execute(
                                    """ select ROUND(sum(o.amount_total),2) from sale_order as o 
                                                        where o.state = 'sale' and o.invoice_status = '%s' and o.team_id in %s and date_order> '%s' 
                                                        and date_order<= '%s'; """
                                    % (self.invoice_status, team_list, day_start, day_end)
                                )

                            else:

                                self.env.cr.execute(
                                    """ select ROUND(sum(o.amount_untaxed),2) from sale_order as o 
                                                        where  o.state = 'sale' and o.invoice_status = '%s' and o.team_id in %s and date_order> '%s' 
                                                        and date_order<= '%s'; """
                                    % (self.invoice_status, team_list, day_start, day_end)
                                )
            ...
            if (
                week_count != (range_value - 1) or (self.total_weeks % 2) == 0
        ...
        for key, value in month_week_total.items():
            ...
            worksheet.write_merge(
                row, row, col, col + 1, str(key) + " Total Sale", right_bold
            )
