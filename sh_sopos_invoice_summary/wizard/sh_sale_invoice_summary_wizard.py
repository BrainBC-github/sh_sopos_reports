...
class SaleInvioceSummaryWizard(models.TransientModel):
    ...
    def print_report(self):
        datas = self.read()[0]
        return self.env.ref(
            "sh_sopos_reports.sh_sale_invoice_summary_action"
        ).report_action([], data=datas)

    def display_report(self):
        datas = self.read()[0]
        report = self.env['report.sh_sopos_reports.sh_sale_invoice_summary_doc']
        ...
        if both_order_list:
            ...
            for rec in both_order_list:
                ...
                worksheet.write(row, 4, str(
                    rec.get('invoice_currency_symbol')) + "{:.2f}".format(rec.get('invoice_amount')), center)
                ...
                worksheet.write(row, 6, str(
                    rec.get('invoice_currency_symbol')) + "{:.2f}".format(rec.get('due_amount')), center)
