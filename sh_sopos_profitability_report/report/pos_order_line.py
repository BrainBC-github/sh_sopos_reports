...
class POSOrderLine(models.Model):
    ...
    sh_margin = fields.Float(
        string="Margin ", compute='_compute_margin_sh', readonly=False, store=True,compute_sudo=True)
    ...
    @api.depends('sh_product_cost', 'price_subtotal_incl','price_subtotal')
    def _compute_profit(self):
        for rec in self:
            rec.sh_profit = rec.price_subtotal - rec.sh_product_cost

    @api.depends('price_unit')
    def _compute_margin_sh(self):
        for rec in self:
            rec.sh_margin = 0.0
            if rec.qty > 0.0:
