...
class ShTspTopSellingProductWizard(models.TransientModel):
    ...
    def display_report(self):
        datas = self.read()[0]
        report = self.env['report.sh_sopos_reports.sh_sopos_selling_pr_doc']
        data_values = report._get_report_values(
            docids=None, data=datas).get('products')
        data_values_amount = report._get_report_values(
            docids=None, data=datas).get('products_qty')
        if self.type == "basic":
            self.env['sh.sopos.selling.product'].search([]).unlink()
            length = len(data_values)
            for i in range(length):
                self.env['sh.sopos.selling.product'].create({
                    'product_id': data_values[i].id,
                    'qty': data_values_amount[i]
                })
            return {
                'type': 'ir.actions.act_window',
                'name': 'Top Selling Products',
                'view_mode': 'tree',
                'res_model': 'sh.sopos.selling.product',
    ...
    def print_top_selling_product_xls_report(self):
        ...
        if self.type == 'basic':
            ...
            for i in final_product_list:
                no = no + 1
                worksheet.write(row, 0, no, left)
                worksheet.write(row, 1, i.name, left)
        ...
        filename = 'Top SO and POS Selling Products.xls'
        fp = BytesIO()
        workbook.save(fp)
